...
instance Functor f => Functor (StateT s f) where
  ...
  (<$>) g (StateT h) = StateT(\x -> mapFst g <$> h x)
    where
      mapFst f (x,y) = (f x,y)
...
instance Monad f => Applicative (StateT s f) where
  ...
  (<*>) (StateT f) (StateT a) = StateT(\s -> (\(g, s') -> (\(b, s'') -> (g b, s'')) <$> a s') =<< f s)
...
distinct' l = eval' (filtering g l) S.empty
  where
    g a = StateT(\s -> pure (S.notMember a s, S.insert a s))
...
distinctF l = evalT (filtering g l) S.empty
  where
    g a = StateT(\s -> if a < 100 then Full (S.notMember a s, S.insert a s) else Empty)
...
instance Applicative f => Applicative (OptionalT f) where
  ...
  (<*>) (OptionalT g) (OptionalT x) = OptionalT(lift2 (<*>) g x)
...
instance Monad f => Monad (OptionalT f) where
  -- (=<<) = error "todo: Course.StateT (=<<)#instance (OptionalT f)"
  (=<<) g (OptionalT x) = OptionalT((\y -> case y of
                                      Empty -> pure Empty
                                      Full z -> runOptionalT (g z)) =<< x)
