...
reverseInteractive = vooid $
  do _ <- putStrLn "Enter source file path:"
     s <- getLine
     _ <- putStrLn "Enter destinatition file path:"
     d <- getLine
     c <- readFile s
     writeFile d $ reverse c
...
encodeInteractive = vooid $
  do _ <- putStrLn "Enter string"
     s <- ((:. Nil) <$>) <$> getLine
     _ <- putStrLn $ show' s
     putStrLn $ join $ (\case
                          (' ':.Nil) -> "%20"
